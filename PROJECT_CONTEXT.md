...
- **⚠️ 확인용 임시 스위치**: 주소 뒤 `?nb=ask`(알림 켜기) / `?nb=ios`(홈 화면 추가)를 붙이면 조건을 전부 건너뛰고 배너가 뜬다(`notifBannerForce()`). 사용자가 디자인을 직접 보려고 요청한 것 — **확인 끝나면 `notifBannerForce()`와 `notifBannerKind()` 첫 줄을 지울 것.**
