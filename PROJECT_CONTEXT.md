...
- **같이 고침**: `bypass:{kcp_v2:{web_siteid:"J26080712852"}}` 추가. **DI 생성에 쓰는 값**이라 안 넘기면 KCP 발급 ID로 대신 만들어져 **DI가 흔들릴 수 있다** — 우리는 그 DI 해시로 중복 가입을 막으므로 값이 고정돼야 한다. ⚠️ 키 이름이 **SDK는 `kcp_v2`, REST API는 `kcpV2`**로 다르다.
...
- **⚠️ 포트원 콘솔에는 본인인증 내역 메뉴가 없다**(사이드바 확인). 실패 원인을 우리가 조회할 방법이 없어서 원인 추적이 오래 걸렸다.
- **아직 확인 안 된 것**: 위 고침 후에도 실패하면 그때가 KCP 계약 쪽 문제(서비스 개시일 미도래·V2 전환 미반영·인증키 불일치)다. **KCP 고객센터 1666-6410**, 문의할 때 **포트원 쪽을 다 확인했다고 먼저 말할 것** — 안 그러면 "포트원에 문의하세요"로 돌려보낸다.
