...
### 프로필 소개 영역에 동작 버튼 모음 + 홈 이중 새로고침 수정 (2026-08-13)
- **프로필 동작 버튼**: 전체 폭 '💬 채팅하기' 버튼과 큼직한 뮤트·차단·메모 상자가 소개 **아래**에 따로 있어 첫 화면을 크게 잡아먹었다. → 소개 영역 안(`.pfh-acts`)에 **작은 알약 4개**(채팅·뮤트·차단·메모)로 모았다. 메모는 버튼으로 **접었다 펴는** 방식 — 소개 안에 입력칸까지 두면 자판이 올라올 때 화면이 통째로 밀린다.
  - **⚠️ `renderUserNoteBox`가 `renderPfHeroActions`도 함께 부른다.** 토글 뒤 다시 그리는 호출부가 여러 곳이라, 한쪽만 갱신하면 "차단했는데 위 버튼은 그대로"가 된다.
  - **⚠️ 차단 중에는 뮤트 버튼을 내린다** — 차단이 뮤트를 포함하므로 둘 다 두면 뜻이 겹친다.
  - **⚠️ `.unote[hidden]{display:none}`** 이 없으면 접어 둔 메모 칸이 그대로 펼쳐져 나온다(커미션 서식 줄에서 겪은 것과 같은 함정).
- **🐛 홈 탭이 두 번 새로고침되던 문제**: `goHome()`이 `selectBoard("all", skipRender)`로 한 번 그린 뒤 `refreshFeed(true)`가 **force**라 내용이 같아도 또 그렸다. 다른 탭에서 돌아오면 `skipRender=false`라 두 번 그려져 "두 번 새로고침"으로 보였다. → **`refreshFeed(onHomeFeed)`** — 이미 홈에 있을 때만 force(그때는 "새로 보여줘"라는 명시적 요청이라 눈에 보여야 한다). 실측: 다른 탭→홈 **1회**, 홈→홈 **1회**.

### 🐛 커미션 설명 서식 도구 3건 수정 (2026-08-13, 사용자 신고)
바로 앞에서 붙인 서식 도구에서 나온 문제들. 셋 다 한 뿌리에서 나왔다.
- **① 색·글꼴·크기 줄이 처음부터 다 펼쳐져 있었다.** `hidden`을 걸었는데도 보였다 — **`.cm-reg-toolbar{display:flex}`가 브라우저 기본 `[hidden]{display:none}`을 이긴다.** `.cm-reg-toolbar[hidden]{display:none}`을 명시해야 한다. ⚠️ 글쓰기 쪽 `.ed-fmt-row[hidden]{display:none}`에는 원래 있던 규칙인데 새로 만들면서 빠뜨렸다.
- **② 색·글꼴·크기를 눌러도 안 먹혔다.** ①의 결과로 사용자가 `색 ›`을 거치지 않고 곧바로 견본을 누르는데, 그러면 저장된 선택 영역이 없어 **글 맨 끝에 서식이 걸려** 아무 일도 안 한 것처럼 보였다. → 하위 줄의 **모든 버튼이 mousedown에서 `cmDescSaveSelection()`을 먼저** 부르게 했다.
- **③ 목록 점(•)이 입력창 밖으로 삐져나왔다.** 전역 리셋 `*{padding:0}`이 `ul`의 기본 안쪽 여백을 지워, `list-style:outside` 점이 글자보다 왼쪽 바깥에 그려졌다. `.cm-reg-editor ul,ol{margin-left:24px}`로 자리를 확보(글쓰기 `.ed-content ul`과 같은 방식). **상세 표시(`.cm-desc`)에도 같이** 넣었다 — 편집기만 고치면 저장 후에 다시 삐져나온다.
- **함께 점검(모두 정상)**: 굵게·기울임·밑줄·취소선 / 서식 겹쳐 적용(색+크기 중첩) / 저장 경로에서 색·크기 보존 / 아무것도 선택 안 하고 미리 골라 이어 입력 / 도구 줄 2줄로 접히고 잘림 없음.
...
### 🐛 커미션 목록 썸네일이 뭉개지던 문제 (2026-08-15, 사용자 신고 — 내 회귀)
"목록에서만 화질이 깨져 보인다."
- **원인**: 어제 만든 썸네일 규격 **360px/q0.72가 화면 배율을 빼먹은 값**이었다. 카드는 165 CSS px로 그려지지만 실제 필요한 건 배율 2배 화면 330px, 3배 495px. 게다가 카드는 정사각으로 잘라 쓰므로(object-fit:cover) 세로 이미지는 **짧은 변**이 기준. 실측(cm13): 원본 1800² → 썸네일 360² → **3배 화면에서 38% 확대**. 상세는 원본을 써서 멀쩡했기에 "목록에서만"으로 보였다.
- **수정**: 긴 변 **720px / q0.82**, 원본이 이미 작으면 확대하지 않음. 실측 1500×2000 → 540×720, **4KB**(절감 효과 유지).
- **⚠️ 세대 번호 도입(`.thumb.webp` → `.thumb2.webp`)** — 규격만 바꾸면 **이미 올라간 옛 썸네일이 계속 뭉개진 채로 쓰인다**. 번호를 올리면 옛 파일은 404 → `thumbFail`이 원본으로 바꿔 끼워 **즉시 선명해지고**, 새 업로드부터 새 규격이 생긴다. 조사 결과 옛 썸네일이 있는 건 cm13 하나뿐이었다(나머지는 기능 도입 전 업로드라 이미 원본 사용 중).
- **⚠️ 접미사는 세 곳이 일치해야 한다**: `palo.js THUMB_SUFFIX` · 업로드 라우트 · **삭제 라우트(세대 전부 지움 — 빼먹으면 원본 삭제 후 고아 파일이 영영 남는다)**.
- 검증(dev): 세대 접미사·중복적용 방지·외부주소 무시, 3:4 원본 → 짧은 변 540px(배율3 기준 495px 충족), 작은 원본 확대 안 함, 옛 썸네일 404 → 원본 폴백 실동작.
...
- **덤**: 두 자리가 비면서 직전에 320px 때문에 접어 뒀던 검색·랭킹·내 커미션 규칙이 불필요해져 제거 — 좁은 폰에서도 상세에 4개가 다 보이고 넘침 0(실측 320·375).
- **⚠️ PC는 제외(`max-width:860px`)** — PC는 하단 탭이 없고 상세에서는 ☰(서랍)마저 ←로 바뀌어 있어, 여기서 알림·프로필까지 숨기면 **갈 방법이 아예 사라진다**(실측: menuBtn 폭 0, 탭바 none). 모바일은 하단 탭 '내 정보'가 남아 있어 안전.
- 검증: 320/375/1440 × 홈·목록·상세 전부 넘침 0, PC에서 알림·프로필 유지, 홈은 그대로.
...
- **빈 칸 표시**: `.cm-reg-miss` 붉은 테두리 + 2회 펄스(reduced-motion 존중). **⚠️ 표시는 등록을 눌렀을 때만 붙는다** — 입력하는 족족 안 간 칸까지 붉어지면 혼내는 폼이 된다. 채워지면 cmCheckReg가 그 칸만 걷는다(지우기 전용).
- **자동 스크롤**: 첫 빈 칸으로 smooth 이동.
- `cmRegMissing()`이 {label, el(id)} 목록을 돌려주고 활성 판정·문구·표시·스크롤 네 곳이 전부 이것 하나를 쓴다.
- 검증(dev): 복수/단수 문구, 4곳 표시→제목 채우면 그 칸만 걷힘→전부 채우면 0+활성. 스크롤은 숨긴 패널에서 검증 불가(**전역 scroll-behavior:smooth가 rAF 기반이라 hidden에서 모든 스크롤이 안 움직임** — scrollTo도 0에 머묾, 환경 제약 계열에 추가).

### 커미션 등록 버튼 — 빈 항목 안내 + 바 위치 최종 결론 (2026-08-15, 사용자 요청)
**① 비활성 버튼을 눌러도 무반응이던 것** → `disabled` 대신 `.is-off` 클래스(모양만 잿빛) + `aria-disabled`. disabled면 클릭 이벤트 자체가 안 와서 이유를 알려줄 수 없다. `cmRegMissing()`이 빈 필수 항목 이름을 돌려주고, **버튼 활성 판정과 클릭 안내가 같은 목록을 쓴다**(두 곳이 따로 살면 어긋난다). 누르면 "샘플 이미지 · 가격을(를) 채워주세요" 토스트. 실제 차단은 `cmSubmitReg` 첫머리 검사.
**② 폼 제출 바의 키보드 처리 — 세 번째 시도가 최종**: ⅰ)display:none → "등록이 안 눌린다" ⅱ)static으로 폼 끝 이동 → "태그 입력하면 버튼이 사라진다"(순간이동) ⅲ)**아무것도 안 함(fixed 유지)** — 키보드 중엔 그 뒤에 가려졌다가(모든 앱의 표준) 닫히는 즉시 제자리. 키보드 중 스크롤의 일시적 떠다님은 iOS 고유 동작으로 수용, 잔류 밀림은 focusout 재정착이 처리. **⚠️ 이 자리는 더 만지지 말 것** — CSS 주석에 시행착오 기록.
- 검증(dev): 빈 상태 클릭→전체 목록 안내, 일부 채움→남은 것만, 전부 채움→is-off 해제, kb-open 중 fixed 유지·하단 위치.

### 커미션 태그 선택화 + 설명 필수 안내 (2026-08-15, 사용자 요청)
- 태그를 필수→선택으로: `cmCheckReg`에서 `tags.length>0` 조건 제거, 라벨 `* → 선택`. 필수는 이미지·제목·가격·설명(+리뷰이벤트 시 혜택).
- 설명 라벨에 "필수 · 입력해야 등록할 수 있어요" 문구 추가.
- **⚠️ 동반 수정**: 상세 화면의 데모 태그 폴백(`['두상','흉상','반신','드림']`) 제거 — 그대로 두면 태그 없는 커미션에 **가짜 태그**가 떠 버린다. 태그가 없으면 태그 줄 자체를 안 그림. (목록·프로필 쪽은 이미 `(d.tags||[])` 가드라 무영향, DB `tags_max5` 체크는 빈 배열 통과)
- 검증(dev): 태그 0개+나머지 충족 → 등록 활성, 설명 비우면 잠김, 미리보기에 가짜 태그·태그줄 없음.

### 🐛 "커미션 등록이 안 눌린다" (2026-08-15, 실사용자 신고 — 내 회귀)
원인: 키보드 중 하단 바 숨김(kb-open) 목록에 **폼 제출 바까지 넣은 것**. 설명을 입력하다(키보드 열림) 등록을 누르려면 버튼이 화면에서 사라져 있었다. 제출 로직 자체는 정상(실서버 재현으로 확인 — 잠금 false·필수값 채우면 활성).
- **수정**: 폼 제출 바 3종(`.cm-reg-bottom`·`.cm-apply-bar`·`.cm-wr-submit`)은 숨기지 않고 키보드 중엔 **`position:static`으로 문서 흐름에 편입** — 폼 끝에 놓여 스크롤로 항상 닿고, 흐름 안에 있으면 떠다니는 게 물리적으로 불가능(원래의 "떠다님" 문제도 재발 없음). 키보드 닫히면 fixed 복귀. 탭바·FAB는 계속 숨김(입력 중 불필요).
- 보강: 저장 중 재클릭 시 "저장 중이에요" 토스트(조용한 무시도 "안 눌린다"가 된다), `cmOpenRegister`에서 잠금 초기화(이전 시도가 네트워크에서 영영 안 돌아온 경우의 고착 방지).
- 검증(dev): 키보드 중 static·설명란 아래 위치·클릭 도달, 닫힘 후 fixed 복귀, 탭바는 숨김 유지, 재클릭 안내, 재진입 잠금 리셋.
- ⚠️ 교훈: "입력 중엔 필요 없다"는 가정은 **탭바에만** 맞았다. 폼의 제출 버튼은 입력의 목적지다 — 숨기면 안 된다.
...
- 검증: 보이는 입력창 전수에서 16px 미만 0개(체크박스 13.3px 2개는 확대 유발 안 함 — 텍스트 입력만 해당), viewport 메타에서 확대 금지 제거 확인, 헤더 높이·가로 넘침 무변화, 댓글창 폭 정상.
- **⚠️ 새 입력창 규칙**: font-size 16px 미만 금지 — layout.js viewport 주석에 경고.
...
- **약관 (제4조 제6항 신설, 시행 2026-09-14)**: 성인 게시판 = 본인확인 완료 만19세+ 전용, 1년 재확인, 외부 유출 금지, 부정 통과 시 제12조 제재. **불리한 변경 소지라 §3 제3항의 30일 공지 기간 적용** — legal-foot에 조항별 시행일 분리 표기.
- ⚠️ 두 문서 모두 **공지 글이 세트** — 공지 없이 시행일만 바꾸면 §2·§3의 자기 요건 위반. 공지 문안은 사용자에게 전달(관리자 공지 기능으로 게시).
...
- `isFull`이 참이 되면 "남은 N장은 올리지 않았어요" **한 번만** 알리고 멈춘다(장마다 경고 반복 방지).
- 5개 입력에 `multiple` 추가. 각 change 핸들러는 한 줄로 통일.
- **단일 유지(의도적)**: 배너 광고 2곳(배너는 1장), 채팅 사진, 프로필 아바타 — 성격상 한 장.
- 검증(dev): DataTransfer로 가짜 3장 → 순서대로 3회 업로드, 9/10 상태에서 3장 → 1장만 올리고 멈춤, 등록 화면의 입력 2개 `multiple=true` 실렌더 확인.
...
- 상태는 `IV={list,i,wheelAt,swipedAt}`. `ivShow()`가 사진·카운터·좌우 버튼을 한 번에 맞춘다.
- **넘기는 방법 네 가지**: 휠/트랙패드(220ms 스로틀, 세로·가로 어느 쪽이든), 손가락 좌우 스와이프(40px 이상 + 세로 이동보다 커야 함), 화살표키, 좌우 버튼. 양 끝에서 순환(`cmSliderMove`와 같은 규칙).
- **⚠️ 스와이프하면 손가락을 뗄 때 click도 같이 온다** → 그냥 두면 넘기자마자 창이 닫힌다. `IV.swipedAt`을 찍고 `ivBackdrop()`이 400ms 안이면 닫지 않는다.
- **⚠️ 좌우 버튼은 `ivMove`에서 `stopPropagation()`** — 안 하면 바깥 클릭(=닫기)으로 번진다.
- **⚠️ `.iv-nav[hidden]`/`.iv-count[hidden]`을 CSS에 따로 적었다** — `display:flex`가 브라우저 기본 `[hidden]{display:none}`을 이긴다(툴바에서 이미 겪은 함정).
- 목록은 **인라인 onclick 문자열에 배열을 박지 않고** `cmDetailImages()` 호출로 넘긴다(따옴표 이스케이프가 두 겹이 되면 깨진다). 등록 미리보기면 `cmPreviewObj.images`를 본다.
- 검증(dev): 6장짜리 커미션에서 다음/이전/끝에서 순환, 휠 이벤트, 버튼 클릭(안 닫힘), 배경 클릭(닫힘), 한 장일 때 버튼·카운터 `display:none`, 360px에서 좌우 버튼이 화면 안, 스와이프 후에도 열려 있음.

### 말 한마디 없는 채팅방 숨기기 (2026-08-14, 사용자 요청)
"채팅하기"를 누르는 순간 `conversations` 행이 먼저 만들어지는 구조라, 눌러만 보고 나와도 빈 방이 목록에 남았다.
- `openChatList`가 이미 받아 오는 `lastMsgByConv`로 **표시만 거른다**(`convs`·`partnerIds`를 함께 다시 만든 뒤 캐시에 저장).
- **행을 지우지 않는다** → ①예전에 만들어진 빈 방도 그대로 사라진다(소급 적용, SQL 불필요) ②나중에 실제로 말을 걸면 `openOrCreateConversation`이 같은 행을 다시 찾아 써서 대화가 이어진다.
- 관리자 채팅 조회(`chat_admin_access_logs` 쪽)는 건드리지 않았다 — 운영용은 다 보여야 한다.
...
### 🐛 커미션 검색창 포커스 시 화면 확대 — 뷰포트 제한 해제의 남은 낙진 (2026-08-15, 사용자 신고)
**iOS는 16px 미만 글자의 입력칸에 포커스하면 화면을 자동 확대한다.** 이 동작은 원래 뷰포트의 확대 제한(maximum-scale)이 눌러 두고 있었는데, **핀치 줌을 허용하려고 제한을 푼 순간(2026-08-14) 살아났다.** 그때 신고된 등록 폼만 16px로 고쳐서, 나머지 작은 입력칸들이 하나씩 신고되는 중이었다 — 이번이 검색창(13.5px).

- **전수 조사 결과**: 명시 규칙이 16px 미만인 곳 = `.cm-search input`(13.5)·`.cm-wr-text`(14.5) 둘뿐. 그러나 **font-size 규칙이 아예 없는 입력칸은 브라우저 기본값(~13px)이라 역시 확대를 유발** — 채팅 검색·댓글창·메모·관리자 입력 등 다수.
- **고침 2단**: ①명시 규칙 2곳은 `@media(pointer:coarse)`로 16px 오버라이드(PC 디자인 불변) ②규칙 없는 것들은 **`:where()` 안전망** — `@media(pointer:coarse){:where(input:not(체크박스류),textarea,[contenteditable])​{font-size:16px}}`. `:where`는 우선순위 0이라 **명시된 크기(제목 25px 등)는 절대 못 건드리고, 크기를 안 정한 곳만** 받친다. 상속보다는 이긴다(상속은 어떤 선언에도 진다) — 그래서 작은 글자 컨테이너 안의 입력칸도 잡힌다.
- **⚠️ 안전망이 못 잡는 것**: 명시적으로 16px 미만을 준 규칙. 새 입력칸에 작은 글자를 주고 싶으면 **PC 전용 media 안에서만** 줄 것.
- **실측**: 터치 에뮬레이션에서 검색창 13.5→16, 규칙 없는 input/textarea/contenteditable 전부 16, 제목 22px·체크박스 13.3px 유지. PC(pointer:fine)는 전부 원래 값.
- **교훈(뷰포트 제한 해제 같은 전역 스위치를 바꿀 때)**: 그 제한이 **억눌러 주고 있던 동작 전부**가 영향 범위다. 신고된 화면 하나만 고치면 나머지가 시한폭탄으로 남는다 — 처음부터 전수 조사했어야 했다.

### ⚡ 로딩 가속을 전 탭으로 확장 (2026-08-15, "다른 탭도 똑같이")
커미션에 적용한 즉시 표시+프리페치를 나머지 화면에도. 화면마다 방식이 다르다 — **이유가 있어서 다르다:**

- **채팅 목록**: 데이터 로더를 `loadChatListData()`로 분리(openChatList와 유휴 프리페치가 공유), 부팅 4.5초 뒤 미리 불러 메모리 캐시(`chatListCache`)를 채움. **⚠️ 채팅은 커미션과 달리 localStorage에 저장하지 않는다** — 마지막 메시지 미리보기가 담기는 사적 데이터라 로그아웃 후 기기에 남으면 안 된다(메모리 캐시는 applySession이 계정 전환·로그아웃 때 비운다).
- **내 정보/출석**: 화면 자체는 원래 메모리에서 즉시. 출석 카드가 60일치 조회를 기다렸는데, 부팅 6초 뒤 `AT.days`를 미리 채워 카드가 즉시 그려짐(atFillCard는 `AT.days===null`일 때만 조회하므로 자동 연동).
- **이모티콘 마켓**: `reloadEmoticonMarket(skipRender)` 인자 추가로 프리페치(7.5초 뒤), 재방문 시 `EMO_MARKET` 있으면 즉시 렌더 후 배경 갱신. **🐛 겸사 수정**: `renderEmoticonMarket()`이 화면 검사 없이 `#main`을 덮어쓰고 있었다 — 로드 중 다른 화면으로 가면 끝나자마자 그 화면을 이모티콘 마켓으로 클로버(navSeq 가드 계열의 구멍). 화면 스택 top이 `emoMarket`일 때만 그리게 가드 추가.
- **랭킹**: 기간별 메모리 캐시(`LB_CACHE`) — 재방문 즉시 렌더 후 배경 갱신. **🐛 겸사 수정**: RPC를 기다리는 동안 다른 화면으로 가도 끝나면 덮어쓰던 문제 → 랭킹은 화면 스택을 안 쓰므로 **주소(`/ranking`)로 판정**해 막음.
- 프리페치 체인(`cmIdlePrefetch`): 커미션 2.5s → 채팅 4.5s → 출석 6s → 이모티콘 7.5s, 각각 requestIdleCallback + "이미 로드됐으면 건너뜀" 가드. saveData면 전부 안 함.
- **실측**: 프리페치 체인 후 커미션 7건·비로그인 가드 정상, 이모티콘 렌더 가드 동작(다른 화면 덮어쓰기 차단), 랭킹 재방문 즉시 렌더. 이모티콘 0팩은 DB에 공개 팩이 없어서다(정상).
...
- **구멍 ① 밀림(pan residue) 복구가 focusout 한 곳에만 있었다.** 이미 주석에 적혀 있었듯 "포커스된 입력칸이 화면 교체로 사라지면 focusout이 안 오는" 브라우저가 있고, 그 대비책인 `syncKbOpen`은 **kb-open 클래스만 지우고 밀림은 복구하지 않았다.** 검색창에 입력하다 카드를 눌러 상세로 가면 정확히 이 경로 → 밀림이 남은 채 상세의 `.cm-fab-wrap`이 떠다녔다. **고침**: 복구를 `settleViewport()`로 추출해 ①focusout ②syncKbOpen(화면 전환) ③`visualViewport` resize 350ms 디바운스(마지막 안전망 — openPost처럼 enterScreen을 안 쓰는 전환까지 커버) 세 곳에 걸었다. **⚠️ resize 쪽에서는 절대 body.kb-open을 만지지 않는다** — 2026-08-14의 이중 주인 사고(위 fitChatRoom 주석) 재발 방지. settleViewport는 상태를 안 바꾸는 멱등 동작이라 몇 번 불려도 안전하다.
- **구멍 ② `--cm-tabbar-h`가 0에 고착되는 경로.** 이 값을 재는 MutationObserver가 childList만 감시했다: 입력 중(kb-open, 탭바 display:none) 토스트 등으로 DOM이 바뀌면 높이가 **0으로 측정**되는데, 키보드가 닫혀 탭바가 돌아오는 건 **body class 변화뿐**이라 아무도 다시 재지 않았다 → 하단 바들이 탭바 뒤에 깔림. **고침**: body 자신의 class를 보는 옵저버를 **따로** 추가. **⚠️ 기존 옵저버에 attributes를 합치면 안 된다** — subtree와 결합돼 모든 하위 요소의 class 변화(좋아요 토글 등)마다 offsetHeight를 읽어 레이아웃을 강제한다. 루프는 없다(toggle·측정 모두 값이 같으면 안 쓴다 — 수렴).
- **실측**: kb-open+DOM변경 → `67px→0px` 붕괴 재현 → class 복원 후 `67px` 복구 ✅(수정 전엔 0 고착). class 200회 토글 126ms·프리즈 없음. FAB가 탭바 위에 정상 배치.
- **⚠️ 입력 중(키보드 열림)에 폼 제출 바가 떠 보이는 것 자체는 남는다** — iOS가 fixed를 레이아웃 뷰포트에 붙이는 근본 동작이고, 바를 숨기는 방식은 사용자가 원치 않았다(2026-08-14 태그 입력 신고). 이번 수정은 **키보드가 닫힌 뒤에도 남던 잔상**을 없애는 것.

### 🐛 부팅 경합 — /commission 새로고침이 홈으로 튕기던 버그의 진짜 원인 (2026-08-15 해결)
오랫동안 재현을 못 했던 버그("로그인 상태에서만, 가끔"). **원인은 부팅 경합이었다.**

- 로그인 상태의 부팅에서 `applySession`이 **두 번** 돈다: `onAuthStateChange(INITIAL_SESSION)` 이벤트에서 한 번 + `initAuth`의 직접 `await` 한 번. 첫 번째 것이 예약한 `loadMyNotes().then(→onHomeListNow() 통과 시 renderList())`가 **`routeDeepLinkEarly`보다 먼저** 끝나면: 아직 아무 화면도 안 연 순간이라 기존 가드 3종(스택·curTab·.detail)이 전부 통과 → `renderList()`가 홈을 그리며 **주소를 '/'로 밀어버림** → 그 뒤 실행된 `routeDeepLinkEarly`는 이미 '/'가 된 주소를 읽어 **딥링크 증발**.
- 로그아웃이면 `loadMyNotes` 자체가 안 돌아 재현 불가(시크릿 창은 정상이던 이유), 네트워크 타이밍에 따라 나거나 안 났다(재현이 안 되던 이유).
- **고침**: `onHomeListNow()`에 **주소 검사** 추가 — `location.pathname`이 `/` 또는 `/board/…`가 아니면 false. 주소는 renderList 자신이 바꾸기 전까지 원래 값이 살아 있으므로, **"이미 화면을 열었는지"가 아니라 "주소가 홈인지"를 보면 시점과 무관하게 막힌다.**
- **⚠️ 교훈**: 배경 갱신 가드는 "지금 열린 화면"(스택·탭 변수)만 보면 부팅 중 빈틈이 생긴다. **주소가 진실의 원천** — 파생 상태보다 주소를 검사할 것.
- **실측**: 경합 순간 흉내(스택 비움+curTab=home, 주소 /commission) → 가드 막힘 ✅ / 정상 홈·게시판 → 렌더 허용 ✅.

### 🐛 알림 배지 '3' 깜빡임 (2026-08-15 해결)
새로고침 직후 알림 배지에 3이 잠깐 보였다 사라지던 버그. `app/body-html.js` 초기 HTML에 **데모 시절 하드코딩된 `<span id="notiBadge">3</span>`**이 그대로 남아 있었다 — palo.js의 `syncNotifBadge()`가 실제 값으로 바꾸기 전까지 그 3이 보였던 것. 빈 값 + `display:none`으로 시작하게 고침. **⚠️ body-html.js의 데모 잔재는 이것 말고도 PC 사이드바 인기글(`#hotList`, 데모 글 3개 + `openPost(7)` 같은 데모 id onclick)이 남아 있다** — 실데이터 로드 전 잠깐 보이는 건 같지만, 클릭하면 데모 글이 열릴 수 있는 짧은 창이 있다(아래 '알려진 이슈' 참고).

### 🚨 썸네일 크기는 '긴 변'이 아니라 '짧은 변'으로 검산해야 한다 (2026-08-15, 사용자 재신고)
백필·캐시 문제를 다 잡고도 **PC 화질이 그대로**였다. 원인은 규격 선택 자체가 틀렸던 것.

- **카드는 정사각으로 잘라 쓴다(`object-fit:cover`). 그래서 실제로 필요한 건 '짧은 변'인데, 썸네일 규격(360/720)은 '긴 변' 기준이다.** 1083×1800 세로 그림이면 sm(긴 변 360)의 짧은 변은 **217px**뿐이다. PC 카드가 250px이니 줄어드는 게 아니라 **확대**된다.
- "PC는 배율 1이니 360이면 충분"이라는 판단이 바로 이 착각이었다. 게다가 같은 날 PC 카드를 219→250px로 키워서 더 나빠졌다.
- **⚠️ 축소배율 수치를 읽을 때 1보다 작으면 축소가 아니라 확대다.** 0.87x·0.81x를 "잘 줄어들었다"고 보고했다가 사용자에게 다시 지적받았다. 1.0 미만은 전부 확대다.
- **고침**: 배율과 무관하게 **lg(720)를 먼저 쓴다**(`thumbChain`). sm은 lg가 없을 때의 보조 후보로만 남긴다.
- **검산표**(짧은 변 기준, 세로 3:5 그림은 짧은 변 ≈ 긴 변 × 0.6):

  | 원본 | sm 짧은변 | lg 짧은변 | PC 250px | 폰 167px×3=501px |
  |---|---|---|---|---|
  | 1083×1800 | 217 | 433 | lg 1.73x ✅ | lg 0.86x ⚠확대 |
  | 732×1065 | 247 | 495 | lg 1.98x ✅ | lg 0.99x ⚠확대 |
  | 1800×1013 | 203 | 405 | lg 1.62x ✅ | lg 0.81x ⚠확대 |
  | 1000×1000 | 360 | 720 | lg 2.88x ✅ | lg 1.44x ✅ |

- ~~남은 문제: 고배율 폰은 lg(720)로도 모자란다~~ → **같은 날 해결: lg를 900px로 올리고 재생성 모드 추가.** 짧은 변 검산(폰 501px 기준): 1083×1800→542(1.08x✅), 732×1065→619(1.24x✅), 1800×1013→507(1.01x✅), 1000×1000→900(1.80x✅). 판 번호 v3(옛 720 캐시 건너뜀).
- **재생성 실행 완료(2026-08-15, 관리자)** — **실측(운영)**: 모든 R2 `.thumb2`가 900 세대(542×900·900×675·900×900·619×900·900×506)로 교체됐고 화면도 lg 수신. 썸네일 화질 건은 **전부 종결**. 옛 Supabase 이미지 1장만 원본 사용(백필 대상 외, 의도된 동작).
- **결정(2026-08-15, 사용자)**: 포인트 랭킹에 출석 점수가 섞이는 것은 **그대로 둔다** — 랭킹을 "종합 활동 순위"로 취급. `get_score_leaderboard`에서 `event <> 'attendance'` 필터를 추가하지 않기로 함(원하면 언제든 추가 가능).
- **재생성 모드**(`regen:"lg"`) — 이미 있어도 다시 만든다. **⚠️ "없는 것"이 줄어드는 구조가 아니므로 진행 커서가 필수다**: 키를 정렬해 순서를 고정하고, 서버가 마지막 처리 키를 `cursor`로 돌려주면 다음 호출이 `after`로 이어 보낸다. 커서 없이 돌리면 같은 앞 8장만 무한히 덮어쓴다. 관리자 화면 「목록용 썸네일 전체 재생성」 버튼.
- **⚠️ 의도적으로 감수한 것**: 가로로 아주 긴 배너형(1500×500)은 짧은 변 300이라 고배율 폰에서 여전히 모자란다(0.60x). 정사각으로 잘리는 구조상 규격을 더 키우는 건 비효율이라 여기서 멈춘다.

### 🚨 이미지 서버가 404까지 1년 캐시하고 있었다 (2026-08-15, 백필 후 발견)
백필을 돌려 썸네일을 다 만들었는데도 **화면은 그대로 원본을 받고 있었다.** 원인을 파고드니 `img.commi.kr`이 **404 응답에도 `Cache-Control: max-age=31536000`(1년)** 을 붙이고 있었다(실측: 존재하지 않는 키로 요청 → `404` + `max-age=31536000`).

- **이게 왜 치명적인가**: 이 프로젝트의 썸네일은 DB에 기록하지 않고 **주소를 유도**한 뒤 404면 원본으로 폴백하는 구조다. 그런데 404가 1년 캐시되면, **썸네일이 없을 때 그 주소를 한 번이라도 요청한 브라우저는 1년 내내 원본을 받는다.** 나중에 썸네일을 만들어도 소용이 없다. 즉 "404 폴백" 전략 전체가 이 헤더 하나로 무력화돼 있었다.
- **⚠️ `fetch(url,{cache:"reload"})`로 덮어써도 `<img>`는 안 고쳐진다.** 이미지 요청은 `no-cors` 모드라 CORS 모드 `fetch`와 **캐시 항목이 따로 논다.** 실측에서 fetch는 200을 받는데 같은 주소의 `<img>`는 계속 원본으로 폴백했다 — 이것 때문에 한참 헤맸다.
- **임시 대응(코드)**: 썸네일 주소에 판 번호(`THUMB_V="?v=2"`)를 붙여 죽은 캐시를 통째로 건너뛴다. **썸네일을 대량으로 새로 만든 뒤에는 이 번호를 올릴 것.** 원본에는 붙이지 않는다(늘 200이라 죽을 일이 없다).
- **⚠️ 근본 해결은 아직 안 됐다 — Cloudflare 캐시 규칙에서 404에 긴 TTL을 주지 않도록 고쳐야 한다.** 그러지 않으면 앞으로도 썸네일 생성이 실패한 이미지는 그 순간부터 1년간 주소가 오염된다.
- 참고: 커미션 목록에는 **R2 이전 시기의 옛 Supabase Storage 이미지**도 남아 있다. `thumbChain`이 우리 이미지 도메인일 때만 접미사를 붙이므로 이런 주소는 손대지 않는다(의도된 동작). 백필도 R2만 훑으므로 이 이미지들은 계속 원본을 쓴다.

### 썸네일 백필 + 커미션 목록 PC 열 수 (2026-08-15)
**백필**(`/api/storage/backfill-thumbs`, 관리자 전용) — 썸네일 기능 이전에 올라간 이미지에 360/720 썸네일을 붙인다.
- **⚠️ 업로드 라우트는 새 키만 발급한다**(남의 파일 덮어쓰기 방지). 그래서 기존 키에 썸네일을 붙이는 일은 이 라우트에서만 가능하고, 그만큼 좁게 잠갔다: ①로그인 ②`profiles.is_admin` ③**쓰는 키는 반드시 접미사가 붙은 것만** — 원본 키는 어떤 경로로도 쓰지 않는다.
- **⚠️ 신원 확인을 저장소 설정 검사보다 먼저 한다.** 다른 라우트는 설정을 먼저 보지만, 관리자 전용이라 익명 호출자에게 서버 상태조차 알려줄 이유가 없다.
- **존재 확인은 HEAD가 아니라 목록 한 번으로.** 원본 N개면 HEAD가 2N번인데, `ListObjectsV2`로 전체 키 집합을 만들면 한 번에 끝난다.
- 한 호출에 8장씩만 처리하고 남은 개수를 돌려준다(`maxDuration=60`). 화면에서 0이 될 때까지 반복 호출. **한 묶음이 전부 실패하면 멈춘다** — 안 그러면 같은 실패를 무한 반복한다.
- `sharp`는 원래 next의 **전이 의존성**이라 언제 사라져도 이상하지 않았다 → `package.json`에 명시하고 락파일도 갱신(안 하면 Vercel의 `npm ci`가 깨진다).
- **실측**: `sharp(...).rotate().resize({fit:"inside",withoutEnlargement:true}).webp({quality:82})`가 1083×1800 → 216×360 / 433×720 으로, 클라이언트 캔버스 결과(217×360)와 같다. 작은 원본은 확대하지 않는다.

**PC 열 수** — 그전에는 1440px 화면에서도 `.cm-root`가 480px이라 640px이 비어 있었다.
- **⚠️ 넓히는 대상은 `.cm-root-list`(목록 전용 표시)다.** 상세·등록 폼·내 커미션도 같은 `.cm-root`를 쓰는데 거기까지 넓히면 글줄과 입력칸이 화면 끝까지 늘어져 오히려 읽기 나빠진다.
- **⚠️ 폭은 카드가 360px을 넘지 않게 잡는다.** 넘으면 PC용 썸네일(360px)을 확대하게 돼 방금 고친 문제가 재발한다. 861~1179px → 3열/800px, 1180px~ → 4열/1060px, 둘 다 카드 약 249px.
- 검색창은 따로 `max-width:420px`로 묶었다 — 안 그러면 1060px짜리 입력칸이 된다.
- **861px 미만은 손대지 않았다** — 모바일의 음수 여백 상쇄 규칙이 그대로 살아 있어야 한다. **실측**: 375px에서 2열·카드 167px·`margin -20px/-18px` 유지, 1000px에서 3열 249px, 1440px에서 4열 250px, 가로 스크롤 없음.

### 🐛 썸네일 2규격 — PC에서 선화가 깨지던 문제 (2026-08-15, 사용자 신고)
**원인은 하루 전 내가 만든 회귀였다.** 규격을 720px로 올리며 접미사를 `.thumb`→`.thumb2`로 **번호를 올렸더니, 이미 올라가 있던 썸네일이 전부 404가 되어** 모든 카드가 1800px 원본을 받고 있었다. 그 뒤로 새 업로드가 없어서 `.thumb2`는 **단 한 장도 존재하지 않았다.**

- **PC가 더 심한 이유**: 커미션 목록은 화면이 넓어도 항상 2열이고 480px 컬럼 안에 갇혀 카드가 **219px**다. 배율 1인 PC에서는 1083px 원본을 219px 칸에 넣느라 **4.95배 축소** — 브라우저가 큰 배율 축소에 빠른 필터를 쓰기 때문에 **선화가 계단처럼 깨진다.** 폰은 배율 3이라 필요 픽셀이 498px이고 축소가 2배 남짓이라 티가 덜 났다.
- **고침 = 한 규격을 고르지 말고 둘 다 만든다.** `.thumb.webp`(360px, 배율 1) / `.thumb2.webp`(720px, 배율 2~3). 업로드 때 **원본을 한 번만 디코딩하고 캔버스만 두 번** 그려 둘 다 올린다(`makeThumbBlobs`). 서명 라우트도 두 개를 내준다(`thumbUploadUrl`/`thumb2UploadUrl`).
- **고를 때는 `devicePixelRatio`가 기준**(`thumbChain`). 배율 1.25 이하 → `[sm, lg, 원본]`, 초과 → `[lg, 원본]`. ⚠️ **고배율에서 sm(360)은 후보에 넣지 않는다** — 500px로 늘리면 원본을 줄여 쓰는 것보다 오히려 더 뭉개진다.
- 404면 다음 후보로 넘어간다(`data-alts`에 남은 후보를 담고 `thumbFail`이 하나씩 꺼냄). 마지막에서 실패하면 `onerror`를 비워 무한 반복을 막는다. 구분자가 공백인 것은 안전하다 — URL에는 공백이 못 들어간다.
- **⚠️ 앞으로 규격을 바꿀 땐 번호를 올리지 말 것.** 접미사를 **새로 추가하고 옛 것을 후보로 남긴다.** 번호를 올리면 그 순간 기존 썸네일이 전부 고아가 되고, 새 업로드가 쌓일 때까지 **모든 사용자가 원본을 받는다.**
- **실측(PC, 배율 1, 219px 칸)**: 축소배율 4.95x → 0.99~1.64x. 썸네일이 아예 없는 옛 이미지 3장은 여전히 원본(2.28~4.63x) — **백필 필요, 아래 남은 이슈 참고.**

### 🎨 새 기능 디자인을 기존 commi 디자인에 맞춤 (2026-08-15, 사용자 지적 "바이브코딩 스러워")
출석체크와 리뷰 이벤트 배너가 사이트에서 혼자 겉돌던 문제. **원인은 하나로 요약된다 — 이미 있는 것을 안 보고 새로 발명했다.**

- **⚠️ 이 사이트에는 이미 확립된 글래스모피즘 레시피가 있다**(파일 끝 '글래스모피즘 마감', 2026-08-08 시안 A 채택). `.pf-group`/`.pfh`/`.login-modal` = 흰색 사선 그라디언트 2겹 + `blur(20px) saturate(2.1) brightness(1.05)` + 안쪽 흰 하이라이트 + 2단 그림자. 안쪽 판(`.pf-list`/`.pf-tile`/`.pf-st`)은 `rgba(255,255,255,.62)` + 2단 그림자. **유리를 새로 만들지 말고 이 값을 쓸 것.**
- **⚠️ 새 카드에는 `.pf-group`을 반드시 함께 달 것.** 유리 마감이 그 클래스에 걸려 있어서, 빼면 내 정보 화면에서 **혼자 불투명한 흰 상자**로 뜬다. 이번에 `.at-card`/`.at-cal`이 정확히 그랬다. 통계 3칸도 `.at-st`를 새로 만들었다가 `.pf-st`로 교체(유리 오버라이드를 못 물려받고 있었다).
- **상징색은 `linear-gradient(120deg,var(--brand),var(--grape))`** — 로고·워드마크·`.write-btn`이 쓰는 값. 주 버튼은 여기에 `border-radius:15px` + `box-shadow:0 4px 14px rgba(255,93,162,.35)`까지 통째로 맞춘다.
- **⚠️ `.chip.on`(파일 앞쪽, 107줄)은 참고하지 말 것.** 같은 그라디언트가 적혀 있지만 **뒤에서 두 번 덮여**(1071·1122줄) 지금은 알약형(테두리+옅은 그림자)이다. 이 파일은 후반 오버라이드가 많아서 **앞쪽 값만 보고 베끼면 죽은 디자인을 되살리게 된다.** 값을 베끼기 전에 반드시 `grep -n "\.클래스명"`으로 전체 등장 위치를 확인할 것.
- **⚠️ 계산 스타일로 검증할 때 `document.querySelector`는 화면에 이미 있는 같은 클래스를 먼저 잡는다.** `.write-btn`을 확인하려다 헤더의 `.write-btn.commission-btn`(배경 없는 변형)을 읽고 "그라디언트가 죽었다"고 오판했다. 격리용 컨테이너를 만들어 그 안에서 조회할 것.
...
### 🐛 PC 넓은 화면에서 채팅방 뒤가 깨져 보이던 문제 (2026-08-15, 사용자 신고)
채팅방(560px 기둥)이 열리면 body의 `cm-page`가 벗겨지는데(8/13 의도된 수정), 그 순간 **뒤에 홈 구조 전체(3단 그리드·양쪽 사이드바·트렌드바)가 되살아나** 기둥 양옆으로 비쳤다. 기둥은 본문 칼럼과 32px 어긋나 있고 경계도 없어 깨진 그림.

- **원인은 두 결정의 시차**: 8/14에 배경막(`.chatroom.open::before`)을 "넓은 화면에선 옆이 보이는 게 정상"이라며 **모바일 전용**으로 뒀는데, 하루 전(8/13) 채팅에서 cm-page를 벗기게 바꾼 것과 합쳐지자 옆에 보이는 게 '깔끔한 한 칼럼'이 아니라 홈 잔해가 됐다. **⚠️ 교훈: "이건 이래도 정상"이라는 판단은 그 전제(당시 화면 상태)가 바뀌면 같이 무효가 된다** — 판단의 전제를 주석에 남길 것.
- **고침**: 배경막을 전 화면 공통으로 + `@media(min-width:601px)`에서 기둥에 좌우 1px 테두리·그림자(민무늬 배경 위에 종이처럼 떠 보이게).
- **실측**: 1600px에서 좌우 모두 배경막(`elementFromPoint`가 chatRoom 반환), 기둥 테두리 1px·그림자 적용, 모바일(375px) 배경막 유지·테두리 없음·가로 스크롤 없음.

### 커미션 자리별 알림 권유 (2026-08-15)
알림을 안 켠 사람에게 커미션 관련 3개 자리에서 상황에 맞는 문구로 권유. `notifBannerHTML(ctx)`로 파라미터화(자격 판정은 전부 `notifBannerKind` — 이미 켬·거절·비로그인·iOS 홈화면 안내·다른 기기 구독까지 기존 로직 재사용).

| 자리 | 대상 | 방식·문구 |
|---|---|---|
| 커미션 등록 화면 | 작가 | 배너 `cmreg` — "문의·신청 알림을 놓치지 마세요" |
| 등록 성공 직후 | 작가 | **확인창**(`cmNudgeNotifAfterRegister`, 토스트 600ms 뒤) — 화면이 '내 커미션'으로 바뀌어 배너로는 순간을 놓친다. **수정일 때는 안 묻는다**(잔소리). iOS는 홈 화면 추가 안내로 분기 |
...
- ~~시험 표식 nb_test~~ — **검증 완료 후 제거(2026-08-15)**. 사용자가 4곳 모두 동작 확인 → 우회 코드 삭제, 이제 자격 판정만 따른다(이미 켠 사람에게는 안 보임). 사용자 브라우저에 남은 localStorage 값은 읽는 코드가 없어 무해. 틱톡용 `tk_test`(`?tk_test=1`)는 심사 승인 전까지 유지.

### 인앱 브라우저 집중 로그인 (2026-08-15)
X(트위터)·틱톡 **앱 안의 브라우저**로 들어온 방문자에게는 로그인 모달에서 **그 서비스 버튼만** 보여준다(`inAppBrowser()` — UA 판별: 틱톡 `musical_ly|bytedance|tiktok|trill`(양 플랫폼 확실), X `twitter`(안드 확실, **아이폰 X앱 최신 버전은 표식이 없을 수 있어 그때는 기본 모달** — 오판별보다 안전)).

- **이유 둘**: ①그 앱 사용자는 그 계정이 이미 로그인돼 있어 한 번에 끝난다 ②**구글 로그인은 인앱 웹뷰를 구글이 차단**(`disallowed_useragent`)해서 어차피 자주 실패한다.
- **⚠️ 그 서비스 로그인이 꺼져 있으면(심사 전 틱톡 등) 집중 모드를 켜지 않는다** — 버튼이 하나도 없는 모달이 된다. 실측으로 확인.
- **⚠️ 「다른 방법으로 로그인」 링크를 반드시 남긴다** — 구글로 가입한 기존 회원이 인앱으로 들어왔을 때 로그인 길이 막히면 안 된다. 누르면 페이지를 떠날 때까지 전체 모달로 복귀(`_lgShowAll`).
- **⚠️ 집중 모드 코드는 `setLoginMode` 뒤에** — setLoginMode가 입력칸 표시를 되돌리므로 앞에 두면 숨김이 풀린다.
- **실측**: UA 샘플 5종 판별 정확, X 인앱→X만·틱톡 인앱(스위치 on)→틱톡만·틱톡 인앱(심사 전)→기본 모달, 다른 방법 클릭→전체 복귀.
...
### 글쓰기 게시판 칩에 성격별 색 (2026-08-16, 사용자 요청)
글쓰기의 게시판 칩 띠에 홈 상단 게시판 탭과 **같은 성격별 색**(CHIP_GROUP: 소통 핑크·그림 퍼플·거래 블루·함께 그린·기타 그레이) 적용. `.ed-bchip.g-*` 규칙은 `.chip.g-*`와 같은 값. 실측: 16개 칩 그룹 분포 정상, 홈과 색 완전 일치(rgb 값 동일).

### 글쓰기 개편 2차 (2026-08-16, 사용자 요청)
①서식 줄 재배치 — 자주 쓰는 [굵게·색·글꼴·크기] 앞, [기울임·밑줄·형광펜·목록·인용] 뒤. 색 버튼은 아이콘 → **텍스트 "색"**(무지개 밑줄) ②크리틱·댓글알림 체크박스 제거 — **JS 어디서도 값을 읽지 않는 죽은 UI였다**(edCrit는 쓰기만 4곳 → null 가드). 인신공격·도용·AI 경고문도 제거 ③제거만으로 **모바일 한 화면 수납 달성**(폼 끝 760px ≤ 812px) ④하단 탭바에서 글쓰기 탭 제거(4탭) → **홈 목록 우측 하단 글쓰기 FAB**(커미션 만들기와 같은 모양, renderList가 출력). **⚠️ PC는 헤더에 글쓰기가 있어 FAB 숨김 — `.cm-fab-wrap{display:flex}`가 뒤쪽 소스라 `.cm-fab-wrap.home-write-fab`로 특이도를 올려야 이긴다**(같은 함정 3번째).

### 글쓰기 UI 3종 정리 (2026-08-16, 사용자 요청)
①헤더의 "commi" 워드마크 텍스트 제거(로고 이미지만, alt="commi"로 접근성 유지) ②글쓰기 게시판 선택: 드롭다운 → **좌우 스크롤 칩 띠**(`.ed-boardstrip`/`.ed-bchip`, 16개 게시판, 그룹 제목 없이 한 줄 — 가로 흐름에 제목이 끼면 끊긴다. 수정 모드는 선택 칩을 `scrollIntoView(inline:center)`로 보이게). `refreshBoardLabel`은 라벨 요소가 사라져 **가드 추가**(안내문 갱신만 유지) ③에디터 하단 넣기 도구(사진·이모티콘·링크·파일·투표): 알약(테두리+배경) → **아이콘+글자만**. 실측 10항목 전부 통과.

### 하단 탭바 — 유리에서 불투명으로 (2026-08-16, 사용자 신고)
탭바가 반투명 유리(평균 0.66 + 블러)라 **뒤 내용에 따라 보이는 색이 변했다** — 커미션 탭을 누르면 뒤가 빈 여백(목록 하단 패딩)으로 바뀌며 "살짝 투명해지는" 것처럼 보임. **불투명 단색**(--surface + 윗선 --line + 그림자)으로 교체. **⚠️ 유리(backdrop-filter) UI는 화면 전환으로 뒤 내용이 바뀌는 자리에 쓰면 이런 '색 출렁임'이 생긴다** — 고정 배경 위(채팅 배경막 등)에만 쓸 것. 실측: 그라디언트·블러 제거, 순백 배경, 커미션 전환 후에도 동일.

### 자유 색 선택 — 글쓰기·커미션 설명란 (2026-08-16, 사용자 요청)
~~OS 스펙트럼 팔레트(input type=color)~~ → **같은 날 8×8 색 표 팝업으로 교체(사용자 요청)**. `pickColor(kind,cb)` 인터페이스는 유지해 호출부 무변경. 표 구성: 위 7줄 = 8색상(빨~분홍) × 밝음→어두움 7단계(hsl 68%), 맨 아랫줄 = 무채색 8단계. 가운데 고정 팝업 + 어둠막(누르면 적용 없이 닫힘).
- **⚠️ 견본·어둠막의 onmousedown은 preventDefault** — 에디터의 글자 선택이 풀리면 색이 허공에 적용된다(설명란 견본 버튼들과 같은 규칙).
- 커미션 설명란: 견본 8색은 빠른 선택용으로 유지 + **무지개 버튼**(글자색·형광펜 각각, conic-gradient)으로 자유 선택. 선택 영역은 기존 패턴대로 onmousedown에서 저장.
- 글쓰기: 글자색·형광펜 버튼이 **고정 한 색**(#bf400c/#fbe9c8)이었던 것을 팔레트 열기로 교체(saveEditorSelection→복원→execCommand). 마지막 고른 색은 종류별로 기억.
- 실측: 무지개 버튼 2개, 임의 색 적용, 팔레트 확정→콜백, 마지막 색 기억, 글쓰기 배선(#writeModal 범위 확인 — 같은 title의 커미션 버튼과 혼동 주의).

### 슬라이더 'n / m' 카운터 제거 (2026-08-16, 사용자 신고)
이미지 안으로 옮긴 공유·더보기 버튼(우상단)과 카운터가 겹쳤다 → **카운터 제거**. 위치 표시는 하단 점(dots)이 담당 — **등록 제한이 10장이라 점은 항상 표시된다**(카운터는 10장 초과 대비용이었는데 그 경우가 존재할 수 없었다). 실측: 카운터 없음·점 정상·상단 버튼 3개 유지·점 갱신 로직 정상(hidden 탭에서는 smooth 스크롤·scroll 이벤트가 안 돌아 핸들러 직접 호출로 검증).

### 커미션 가격 범위 + 프로필 문의 가능 시간 (2026-08-16, 사용자 요청)
`docs/sql/price-range-inquiry-hours.sql`.
- **가격 범위**: `commissions.price_max`(선택) 추가. 등록 폼 = 최소[필수] 원 ~ 최대[선택] 원. 검증: 최대가 있으면 정수 + 최소 이상(`cmRegMissing`). 표시는 `cmPriceText(price,priceMax)` 한 곳 — 최대 있으면 "19,000~50,000원", 없으면 예전처럼 "19,000원~"(기존 데이터 완전 호환). 표시처 3곳(상세·등록 미리보기·내 커미션) + 매퍼 2곳(cmRowToData·내 커미션) 갱신.
- **문의 가능 시간**: `profiles.inquiry_hours`(40자 제한 체크). 프로필 편집 모달(소개글·링크 편집)에 입력칸, 히어로의 소개 아래 "🕐 문의 가능: …" 표시(미설정이면 숨김). 본인·남의 프로필 양쪽 콜사이트에 전달.
- **실측**: 범위 표시·최대<최소 검증·비움 통과·상세 표시·히어로 표시/숨김 전부 확인.

### 하단 탭바 아이콘 — 선 방식 통일 + 커미션 팔레트 (2026-08-16, 사용자 요청)
하단 탭 5개가 전부 면 채움(fill)이라 상단 헤더의 선(stroke) 아이콘과 안 맞았다 → 전부 `fill=none stroke` 선 방식으로 교체. 채팅·내 정보는 **앱의 다른 곳(헤더·프로필)이 쓰는 것과 같은 패스**를 써서 일관성 확보. 커미션은 다이아 → **팔레트**(몸통 선 + 물감 점 4개는 fill — 선만으로는 점이 고리처럼 보인다). 실측: 5개 전부 stroke 방식, 팔레트 점 4개, 탭 이동·활성 표시 정상.

### 커미션 상세 헤더 개편 — 작가 이름+구독, 조작 버튼은 이미지 안으로 (2026-08-16, 사용자 요청)
상세에서 헤더 = **작가 이름(좌, 누르면 프로필) + 구독(우)**. 뒤로·공유·더보기는 **이미지 안쪽 상단**의 어두운 반투명 원형 버튼(`.cm-sl-top`, 인스타식)으로 이사.

- 헤더 요소는 body-html에 정적으로 두고(`#cmHeadArtist`/`#cmHeadSub`, 기본 hidden) `cmSyncDetailHead(d)`가 상세를 그릴 때 채운다 — **헤더는 #main 밖 고정 DOM이라 cmDetailHTML 안에서 바로 채워도 된다**(4개 렌더 경로가 전부 cmDetailCurrentId 설정 지점을 지나므로 한 곳에서 처리). 구독 상태는 본문 카드(#cmSubBtn)와 헤더 버튼을 `cmToggleSubscribe`가 동시 갱신. 본인 커미션·미리보기는 구독 숨김.
- 예전 헤더 방식(☰→← 전환, 헤더 공유/더보기 `.cm-detail-btn`)은 상세에서 통째로 숨김. **⚠️ 「내 커미션」 버튼은 `body.cm-page` 노출 규칙(뒤쪽 소스)이 이겨서 `body.cm-page.cm-detail`로 특이도를 올려 숨겼다** — cm-page 계열 헤더 규칙은 순서·특이도 싸움이 반복되는 지점(2026-08-15 주석 참고).
- 이미지 없는 커미션(그라데이션 자리)에도 버튼을 넣는다 — 뒤로 갈 방법이 사라지면 안 된다.
- **실측**: 이름·구독 표시, 로고·☰·내커미션 숨김, 이미지 위 버튼 3개, 뒤로 버튼 → 목록 복귀 + 헤더 원상복구.

### 프로필 커미션 카드에 조회수·북마크 수 (2026-08-16, 사용자 요청)
`pfCmListItemHTML`에 지표 줄 추가(커미션 목록 카드와 같은 선 아이콘 문법). 태그는 원래 표시되고 있었다(태그 입력한 커미션만).
- **배치(같은 날 후속)**: 제목 → 설명 → 지표(조회·북마크) → **태그 맨 아래·10px** 순. 태그는 보조 정보라 눈에 덜 띄게.
- **북마크 수는 커미션 탭 전용 로더에만 있었다** — `cmLoadBookmarkCounts(list)`로 재사용 가능하게 바꿔(인자 없으면 기존처럼 cmData) `pfArtistCommissions`가 자기 목록에 채운다. views는 `cmRowToData`가 원래 실어 온다.

### 커미션 상세 이미지 — 자르지 않고 전체 표시 (2026-08-16, 사용자 요청)
상세 상단 슬라이더(`.cm-sl-item img`)를 `object-fit:cover`(1:1로 잘라냄) → **`contain`**(전체 표시, 남는 부분 공백). 1:1 칸 자체는 유지(슬라이더 구조·레이아웃 안정). 공백은 순백(`--surface`) — 예전 분홍(`--brand-soft`)을 두면 잘린 게 아니라 액자처럼 보이는 느낌이 안 산다. **목록 카드·샘플 그리드는 cover 유지**(썸네일은 정사각 크롭이 관례, 눌러서 전체 보기 가능). **실측**: 가로 1800×1012 → 상하 각 82px 공백·잘림 없음, 세로 1083×1800 → 좌우 각 75px 공백·잘림 없음, 정사각은 꽉 참.
- ~~후속: 모바일 슬라이더 4:3~~ → **같은 날 되돌림(사용자 결정)** — 세로 그림의 좌우 공백이 4:3에선 더 넓어져 이미지가 작게 떠 보였다(어색). 1:1 유지, 통계줄은 살짝 스크롤. ~~모바일에서 슬라이더를 **4:3**으로 낮춤 — 1:1(375px)이면 통계줄(신청 가능·작업 기간)이 첫 화면 밖으로 31px 밀렸다(실측: 바닥 697 vs 하단 바 위 666). 4:3(281px)으로 94px 확보 → 통계줄까지 스크롤 없이 보이고 63px 여유. contain이라 잘림 없음, PC는 1:1 유지.

### 🐛 알림 배너가 첫 화면에서 깜빡였다 사라지던 문제 (2026-08-16, 사용자 신고)
"다른 기기에서 이미 알림을 켠 계정인지"(`push_subscriptions` 조회)가 **비동기**라, 첫 렌더는 모른 채 배너를 그리고 → 조회가 도착해 '켠 계정'이면 지웠다 → 깜빡임.

- **고침 = 순서 반전**: 조회가 끝나기 전엔 `nb-pending`(display:none)으로 **숨긴 채 그려 두고**, '안 켠 계정'으로 판명될 때만 드러낸다('켠 계정'이면 그대로 제거 — 아무것도 안 보였으니 깜빡임 없음). 판명 이후의 재렌더는 pending 없이 즉시 표시. 조회 실패 시엔 드러낸다(영영 숨기는 것보다 낫다 — 다른 자격 검사는 이미 통과한 상태).
- **⚠️ 교훈**: 자격 검사에 비동기 항목이 섞여 있으면 "일단 보여주고 나중에 지우기"는 반드시 깜빡임이 된다 — **모르는 동안 숨기고, 판명되면 드러내기**가 옳은 방향이다.
- **실측**(supabase 스텁): 켠 계정 → 숨김 상태로 그려졌다가 표시 없이 제거 / 안 켠 계정 → 판명 후 표시 / 판명 후 재렌더 즉시 표시.

### 배경 순백화 (2026-08-16, 사용자 요청)
`--bg` `#fbf7f8`(분홍끼 회색) → `#ffffff`. 구조 덕에 변경 지점 3곳뿐: `--bg` + `--bg-veil`(`rgba(255,255,255,.86)` — **반드시 --bg와 같은 색**이어야 경계 띠가 안 생긴다) + `manifest.js background_color`(PWA 스플래시).

- **⚠️ 이제 --surface(카드)와 --bg가 같은 흰색** — 카드 구분은 테두리(--line-2)·그림자가 전담한다. 카드가 묻혀 보이면 배경이 아니라 테두리·그림자를 조정할 것(변수 주석에도 명시).
- 전수 점검 결과 흰 배경에서 구분 수단을 잃는 요소 없음: 글 목록은 구분선 방식, 위젯은 그림자, 칩은 자체 배경, 트렌드바·헤더는 1px 경계선. `--surface-2`(#f5eef2 분홍끼)는 그대로 남아 포인트 역할.
- themeColor(#e07aa6, 브라우저 상단 틴트)는 브랜드색이라 유지.
- **후속(같은 날, 모바일 신고)**: `body::before` 전면 radial 얼룩(분홍·보라·하늘, .04~.05)이 남아 있어 배경이 덜 하얘 보였다 — 분홍끼 배경 시절의 앰비언스 장식이라 **제거**. 작은 화면은 얼룩 하나가 화면 대부분을 덮어 모바일에서 특히 티가 났다. 검색 닫기 페이드 그림자의 옛 배경색(rgba(253,247,249))도 순백으로. **⚠️ 배경색을 바꿀 때는 '배경 위에 깔린 반투명 장식'도 영향 범위다** — 장식은 원래 배경과 섞여 설계된 색이라 배경이 바뀌면 혼자 튄다.

### 커미션 상태 3단계 — 접수중/오픈 예정/마감 (2026-08-16)
`docs/sql/commission-upcoming.sql`. `status`에 `upcoming` 추가(체크 제약은 기존 status 제약을 카탈로그에서 찾아 지우고 재생성).

- **노출 규칙**: 마감만 숨김(기존과 동일), **오픈 예정은 목록·검색·태그 칩에 노출**. 성인 가림막 RPC(`adult_commission_stubs`)도 `status<>'close'`로 갱신.
- **오픈 예정의 동작**: 상세의 신청하기 → `⏳ 오픈 예정`(회색, 누르면 "문의하기로 물어보세요" 토스트 — **문의는 열려 있다**). 신청 가능 통계도 "오픈 예정"으로. 목록 카드 보라 배지, 프로필 커미션 목록·내 커미션에도 배지. 끌올·추천 점수·관리자 추천 관리는 접수중 전용 유지.
- **🐛 검증에서 잡은 버그**: `cmSetStatus`가 `.cm-reg-tg` 전체를 순회해 **성인·리뷰이벤트 토글의 선택 표시까지 벗겼다** — 공용 클래스였다. `[data-st]`로 좁혀 해결. **⚠️ public/ 스크립트는 HMR이 없다** — dev에서 palo.js 수정 후 재검증하려면 반드시 새로고침(안 하면 옛 함수로 검증해 가짜 실패가 나온다).
- **실측**: 목록·검색에 오픈 예정 노출, 마감 숨김, 상세 버튼 잠금+문의 유지, 등록 토글 3개 전환, 성인·이벤트 토글 무영향.
...
- **내 커미션 카드 개편(2026-08-16, "디자인이 뒤쳐진 느낌")**: 오른쪽에 알약 버튼 5개가 세로로 쌓여 카드가 버튼 기둥이 됐던 것(실측 460px급)을 **위 정보 + 아래 균등 액션 바**(구분선 고스트 버튼, 38px)로 재배치 → 카드 141px. **⚠️ 끌올 대기는 버튼이 아니라 상태 옆 칩** — 누를 수 없는 것이 버튼 모양이면 안 된다. 슬러그도 `/이름` 칩으로 상태 줄에 노출(누르면 수정 모달). 긴 제목 말줄임·5버튼 한 줄·광고 잠금 표시 실측 확인.
- **진입점 3곳(2026-08-16 접근성 요청)**: ①내 커미션 목록 「🔗 링크」 ②**상세 소유자 바** — 삭제 버튼 옆 중립 버튼, **주소가 있으면 그 주소 자체가 문구**(`🔗 commi.kr/네이`), 저장 시 문구·주소창 즉시 갱신 ③**수정 화면 상단 바로가기**. 모달의 현재값 조회는 cmMyList→cmData→cmDetail 순(어느 화면에서 열려도 찾아진다).

### 프로필 커스텀 주소(핸들) — commi.kr/<원하는이름> (2026-08-15)
`docs/sql/profile-handle.sql` + `app/[handle]/page.js` + palo.js. 내 정보 → 설정 → 「프로필 주소」에서 설정.

- **형식**: 2~20자, `[a-z0-9가-힣][a-z0-9가-힣_-]{1,19}`. 영문 대문자는 소문자로 접는다. **마침표 불허**(robots.txt 류 파일명과 혼동 방지 — 그 덕에 파일 주소는 형식에서 자동 탈락).
- **⚠️ 예약어가 생명줄** — 핸들은 최상위 경로를 쓰므로 `commission`을 가져가면 그 프로필은 영영 안 열린다(라우트가 우선). 예약어 목록은 **`reserved_handles` 표(서버, 최종 심판)와 palo.js `HANDLE_RESERVED`(안내용) 두 곳** — **최상위 라우트·public 파일을 새로 만들면 둘 다에 추가할 것.** 현재 67개(라우트+파일명+미래 선점 방지+commi/official/staff 등 사칭 방지).
- **경합 안전**: RPC가 exists 검사 후 update하되 `unique_violation`을 잡는다 — 동시에 같은 핸들을 저장해도 `lower(handle)` 고유 인덱스가 최종적으로 막는다. 빈 값 저장 = 핸들 삭제(기본 /user/uuid 복귀).
- **라우팅**: `app/[handle]/page.js` — 최상위 동적 세그먼트. Next가 정적 라우트를 우선하므로 기존 주소는 안 가려진다(실측: /commission·/terms 정상, /someartist·한글 200). 못 찾은 핸들은 404 대신 앱 셸 → palo.js가 "그런 주소의 프로필이 없어요" 토스트 후 홈. generateMetadata가 닉네임 og title + `canonical`(핸들 주소를 정식으로).
- **⚠️ `primeFromCache` 가드에 `getHandleFromPath()` 필수** — 없으면 핸들 주소로 들어왔을 때 홈 캐시를 그리며 주소를 '/'로 밀어 딥링크가 증발한다(부팅 경합 버그와 같은 계열, 이번엔 선제 차단).
- `openUserProfile`은 프로필에 핸들이 있으면 주소를 `/핸들`로(push/replace 모두), 없으면 예전 `/user/uuid`. 커미션 상세→작가(keepStack) 경로는 미리 박힌 /user/uuid를 fetch 후 replaceState로 바로잡는다.
- **실측**: 파서 8케이스 전부 통과(한글 퍼센트 인코딩·대문자 접기·예약어·두 조각·마침표·1자·첫 글자 제한), SQL 실행 전 딥링크는 홈으로 우아하게 복귀.

### 🐛 /commission 새로고침 때 홈이 먼저 비치던 문제 (2026-08-15, 사용자 신고)
`BODY_HTML`이 모든 주소에서 홈 구조(양쪽 사이드바·트렌드바·카테고리바)를 실어 보내는데, 커미션 레이아웃을 만드는 `body.cm-page`는 palo.js 부팅 → **로그인 확인 왕복** → 딥링크 라우팅이 끝나야 붙었다. 로그인 상태 PC에서 그 사이 수백 ms 동안 홈이 보였다가 커미션으로 바뀜.

- **고침 ①**: `BODY_HTML` **맨 앞에 인라인 스크립트** — 주소가 `/commission(/…)`이면 `document.body.classList.add("cm-page")`. 서버 HTML에 실려 **파싱 중 즉시 실행**되므로(뒤의 요소들이 만들어지기 전) 홈 구조가 한 프레임도 안 보인다. ※`dangerouslySetInnerHTML`로 주입된 게 아니라 **SSR 문서에 직접 실린** 스크립트라 실행된다(APP_TAIL의 palo.js와 같은 원리).
- **고침 ②**: `_cmPageNow()` — 스택이 비어 있으면 무조건 false 대신 **주소로 판정**. 안 그러면 부팅 중 MutationObserver 틱이 선반영 클래스를 도로 벗겨 홈이 다시 비친다. **⚠️ 두 곳의 정규식(`/^\/commission(\/|$)/`)은 반드시 같아야 한다** — 어긋나면 붙였다 뗐다 를 반복한다.
- 부수 개선: 글쓰기 에디터(스택 비움·주소 유지)에서 돌아올 때 cm-page가 유지돼 헤더 상태가 더 정확해짐.
- **실측**: SSR HTML에서 스크립트가 `<header>`보다 앞(6971<7551), 클래스 첫 페인트 적용, 스택 빈 상태+DOM 변경 틱에도 유지, 홈 주소+빈 스택이면 정상적으로 벗겨짐.

### 커미션 목록 PC 상단 재배치 — 포스타입식 한 줄 (2026-08-15)
PC에서 검색·정렬 탭이 모바일 배치(검색 위/탭 아래) 그대로 늘어나 있던 것을 포스타입 PC를 참고해 재배치. **포스타입 리퀘스트 PC 실측(1280px)**: 본문 폭 전체를 쓰는 얇은 텍스트 탭 줄(높이 44px·글자 16px·아래 1px 구분선), 검색은 넓게 별도 배치.
- `@media(min-width:861px)`에서 `.cm-top`을 flex-wrap 한 줄로: `[홈 추천 신규 인기] ─공간─ [🔞] [검색 300px]`, 아래 구분선 전체폭. **DOM은 모바일과 동일, order로만 재배치** — 마크업 분기가 없어야 화면 크기를 넘나들 때 입력값·활성 탭이 유지된다.
- 탭은 PC에서 15.5px + 활성 밑줄 2px. **⚠️ `.cm-tabs`에 `align-items:center`를 주면 안 된다** — 탭이 글자 높이(23px)로 줄어 활성 밑줄이 구분선에 안 닿고 공중에 뜬다(실측으로 발견). 기본 stretch로 탭이 줄 전체(44px)를 채워야 밑줄이 구분선에 정확히 닿는다(실측: 탭 바닥 206 = 구분선 206).
- 모바일(≤860px)은 완전 불변(검색 위 343px 전체폭·탭 13.5px) 실측 확인.

### 리뷰 이벤트 배지 🎁만 (2026-08-15 추가 요청)
커미션 **목록 카드**의 배지에서 "리뷰 이벤트" 글자 제거 → 🎁 하나만 담는 28px 원형 칩(`place-items:center`). ⚠️ 글자용 알약 padding을 그대로 두면 이모지 혼자 눌린 타원이 된다 — 고정 지름으로 교체. 뜻 설명은 상세의 배너·태그가 담당(그쪽은 그대로), PC 마우스오버용 `title="리뷰 이벤트 진행 중"` 유지.
...
- **⚠️ 색 얼룩에 `filter:blur()`를 쓰면 모서리에 색 자투리가 삐져나온다**(2026-08-15 사용자 신고). **필터가 걸린 자식은 부모의 둥근 모서리로 잘리지 않고 사각 박스로만 잘린다** — `overflow:hidden`을 줘도 소용없다. 블러 대신 **모서리에 앵커를 둔 큰 radial-gradient**(`125% 135% at 0% 0%` 식)로 부드러움을 만들고, 두 층 모두 `inset:0` + `border-radius:inherit` 로 **스스로 둥글게 잘리게** 한다. 덤으로 큰 영역 블러가 사라져 모바일에서 훨씬 가볍다. ※`clip-path:inset(0 round 20px)`으로도 막을 수 있지만 그러면 **바깥 그림자까지 잘려나간다** — 쓰지 말 것.
...
### 버튼 디자인 시안 C 확정 + FAB 펼침→접힘 (2026-08-17)
아래 전환기로 비교한 끝에 **사용자가 C(잉크 미니멀)를 선택**, 단 두 가지 주문과 함께:
① FAB는 "처음에는 펼쳐져(아이콘+글자) 있다가 시간이 지나면 접혀서 동그랗게" ② "배경색을 카페(사이트)에 맞게" — C의 잉크색 대신.
- **FAB(.cm-fab)**: 높이 56px 알약(아이콘 22px+`.cm-fab-txt` 글자) → `fabAutoCollapse()`가 2.6초 뒤 `.mini`로 접어 56px 원형 아이콘만 남김. 배경은 사이트 상징색 그라디언트(`linear-gradient(120deg,var(--brand),var(--grape))`). 중립 윤곽 2겹 그림자 유지.
  - **auto 폭은 CSS 전이가 안 걸린다** → 접기 직전 `offsetWidth`를 픽셀로 고정하고 reflow 후 `.mini`+`width:56px`. 글자는 `.cm-fab-txt{max-width}` 전이로 접힘.
  - **한 번 접힌 버튼(글자 기준 `_fabMiniDone`)은 재렌더 때 처음부터 접힌 채** — 홈 목록은 수시로 다시 그려져 매번 펼치면 들썩임. 홈 "글쓰기"와 커미션 "커미션 만들기"는 키가 달라 각각 한 번씩 펼쳐짐. 호출부는 `renderList()` 끝과 `openCommissionList()`.
- **구독(.cmh-sub·.cm-sub-card .cm-btn)·팔로우(.d-follow)**: C대로 잉크 알약(`var(--ink)`=#3a2c36, 999px), 누른 상태는 투명 배경+`var(--muted)` 글자+`inset 0 0 0 1.5px var(--line-2)` 테두리. `.cm-sub-card .cm-btn.on` 규칙은 이번에 신설(전엔 글자만 바뀌었음).
- **전환기·시안 B/D/E CSS·`?btn` IIFE 전부 제거**(뒷정리 규칙 이행). `localStorage.removeItem("palo_btn_v")` 한 줄로 기기에 남은 저장값 청소.
- 검증(375px): 홈 111×56 펼침→56×56 원형·글자 접힘, 커미션 151px 펼침→원형, 재렌더 시 처음부터 mini, 구독·팔로우 셋 다 잉크 999px + on 상태 테두리형, `btn-v`/`palo_btn_v` 잔재 0 (`public/palo.min.js`는 빌드가 재생성하는 압축본이라 무관).

### 🐛 글쓰기 게시판 칩 띠 좌우 잘림 (2026-08-17)
`.ed-boardstrip`이 `.ed-body`의 좌우 여백 안에서 스크롤돼, 칩이 본문 가장자리에서 뭉텅 잘려 보이던 문제. `.cm-chips`와 같은 full-bleed 관용구로 수정 — 본문 여백만큼 음수 마진으로 밀고 같은 값을 안쪽 여백으로(기본 22px / ≤860px 16px / **≤400px 18px** — 이 구간은 `.wrap{padding:0 18px}`(798줄)이 소스 순서상 `.ed-body`(529줄)보다 뒤라 좌우 여백이 18px가 됨, -16으로 하면 잔재 2px 남는 것 실측). 검증: 375px에서 띠 0~375 전폭, 첫 칩 18px 시작, 끝 스크롤 시 마지막 칩 잘림 없음; 1280px에서 마진/패딩 -22/22 상쇄·가로 넘침 없음.

### 글쓰기 글자색·형광펜 → '색상' 버튼 하나로 통합 (2026-08-17, 사용자 요청)
서식 줄의 '색'(글자색)과 '형광펜' 버튼을 **텍스트 버튼 '색상' 하나**로 합침. 누르면 8×8 색 표 팝업이 뜨고, **팝업 위 [글자색|형광펜] 탭**에서 종류를 고른 뒤 색을 찍는다.
- `pickColor(kind,cb,opts)`에 `opts.tabs` 추가 — 탭은 통합 버튼(`edPickColor()`, 인자 없음)으로 열 때만 보이고, 커미션 설명란의 단독 호출(`cmDescPickColor/PickHilite`)은 종전대로 색 표만 뜬다(콜백 2번째 인자로 종류가 넘어가지만 단독 호출은 무시).
- `_edColorKind`가 마지막 종류를 기억 → 다음에 그 탭이 켜진 채 열림. 탭 버튼도 `onmousedown=preventDefault`(선택 유지 규칙).
- 검증: 서식 줄 [굵게][색상][글꼴][크기]|[기울임][밑줄][목록][인용], 형광펜 적용(background-color)·글자색 중첩 적용·종류 기억·단독 호출 탭 숨김.

### 하단 탭 강조를 에브리타임식으로 전환 (2026-08-17, 임시 전환 — 복구 가능)
유리 조각(#tabInd)이 미끄러지는 이동 효과 대신, 평소엔 옅은 색(`var(--muted-2)`)이다가 선택된 탭만 진한 색(#9c3566)이 되는 방식으로.
- **기존 코드는 하나도 안 지움** — `palo.js` 상단 `TABBAR_STYLE` 상수가 `"simple"`이면 `body.tab-style-simple` 클래스가 붙고, CSS가 그 클래스 아래에서만 `.tab-ind{display:none}`+색 강조를 적용. **되돌리기 = `TABBAR_STYLE`을 `"glass"`로** 바꾸면 유리 조각·출렁임(wobble)·아이콘 들썩임이 전부 그대로 복귀.
- `syncTabInd()`는 계속 돌지만 표시선이 display:none이라 무해. 색 전이는 `.tab`의 기존 `transition:color .26s`가 담당.
- 검증(375px): 클래스 부착·표시선 숨김·선택 156,53,102/비선택 199,188,196·탭 전환 시 색 교대·아이콘 transform none.

### 하단 탭 '내 정보' 삭제 → 3탭 (2026-08-17, 사용자 요청)
하단 탭바가 홈·커미션·채팅 3탭이 됨. 프로필 진입은 **헤더의 내 프로필 아이콘**으로 일원화.
- **동반 수정(전제 붕괴)**: 커미션 화면(모바일)에서 헤더의 알림·프로필을 접었던 8/15 결정은 "하단 탭 '내 정보'로 갈 수 있다"는 전제였다 → 탭 삭제로 전제가 깨져 **프로필 아이콘만 되살림**(알림은 계속 접음). 지금은 이 아이콘이 커미션 화면의 유일한 내 정보 입구.
- `syncTabs("me")` 호출부(openProfile 등 다수)는 그대로 둠 — 해당 탭이 없으면 전 탭이 꺼지고 표시선(#tabInd)도 `if(!on)` 분기로 자동 숨김이라 안전(실측: 내 정보 화면에서 활성 탭 0·표시선 opacity 0).
- 검증(375px): 탭 3개, 커미션 탭 이동 시 표시선 정렬(left/폭 일치), 커미션 화면 프로필 보임·알림 숨김, 홈 복귀 정상.

### (기록) 버튼 디자인 시안 라이브 전환기 (2026-08-17, 임시 — C 확정으로 제거됨)
"글쓰기·팔로우·커미션 만들기 버튼 디자인이 별로"라는 피드백 → HTML 비교 페이지 5종(A현행 그라디언트/B솔리드/C잉크 원형/D뉴트럴 화이트/E소프트 파스텔)을 보여주자 "각각 버전을 다 만들어서 실제로 비교하게 해줘" → **2026-08-08 `?skin` 전환기와 같은 방식**으로 실사이트 전환기를 넣음.
- `commi.kr/?btn=b|c|d|e`로 켜고 `?btn=a`로 원복. localStorage(`palo_btn_v`)에 저장 → `body.btn-v-*` 클래스. 파라미터는 즉시 주소에서 지움. 일반 사용자 무관.
- 대상: `.cm-fab`(글쓰기·커미션 만들기 FAB), `.cmh-sub`(상세 헤더 구독), `.cm-sub-card .cm-btn`(상세 카드 구독), `.d-follow`(글 상세 팔로우).
- ⚠️ **하나 고르면 globals.css의 `btn-v-*` 블록 전체와 palo.js의 `?btn` 처리 IIFE를 지우고, 고른 값만 원래 규칙에 반영할 것**(skin 전환기 때와 동일한 뒷정리 규칙).
- **FAB 중립 윤곽 그림자 (2026-08-17 추가)**: FAB 그림자가 자기 색 글로우뿐이라 비슷한 색 배경 위에서 묻힌다는 피드백 → `.cm-fab` 기본·호버와 시안 B~E의 `box-shadow` 전부에 중립 잉크 윤곽 2겹(`0 0 0 1px rgba(31,22,28,.08)` + `0 1px 4px rgba(31,22,28,.14)`)을 색 글로우 앞에 추가. 호버 규칙이 그림자를 통째로 갈아끼우므로 호버에도 같은 윤곽을 포함해야 함. 하나 고르고 정리할 때 이 윤곽은 살릴 것.

### 🐛 맨 위로 버튼과 떠 있는 버튼(FAB) 겹침 (2026-08-17)
홈 글쓰기 FAB(2026-08-16 신설)와 `.toTop`(우측 하단, bottom 96px)이 모바일에서 같은 자리 — 커미션 목록의 '만들기' FAB도 원래부터 같은 문제였음.
- **수정**: `@media(max-width:860px){body:has(.cm-fab-wrap) .toTop{right:18px;bottom:calc(var(--cm-tabbar-h,0px)+16px+56px+12px)}}` — FAB는 `#main`과 함께 사라지므로 `:has()`가 곧 "지금 화면에 FAB가 있다"는 뜻. 여유 56px는 시안 C(원형 56px) 기준. FAB 없는 화면(글 상세 등)은 96px 유지, PC는 FAB가 중앙 컬럼 옆이라 원래 안 겹쳐서 손 안 댐.
- 검증(375px): 홈·커미션 목록 모두 맨 위로 버튼이 FAB 위 24px(오른쪽 끝 정렬 357=357), 글 상세는 96px 유지.
- 📌 **`.toTop`은 `transition:.22s`(all)라 값을 바꾼 직후 동기로 읽은 `getComputedStyle`은 전이 시작값(옛 값)을 돌려준다** — 2026-08-08에 기록해둔 "숨겨진 창에서 transition 함정"과 같은 계열인데 또 걸림. 규칙이 안 먹는 줄 알고 한참 헤맴. **측정 전 `transition:none!important`를 잠깐 넣거나 전이 시간만큼 기다린 뒤 잴 것.**
