...
- **남은 것**: ①포트원 채널의 **'본인인증 인증키'(= KCP 사이트키 `site_key`)** 입력 — KCP에 재안내 요청 중 ②Vercel 환경변수 `PORTONE_API_SECRET`·`ADULT_ID_SALT` ③테스트 인증 1회 ④`ADULT_BOARD_ENABLED=true`.
- **⚠️ ④는 마지막에.** 네이버 로그인 검수(소명서에 '성인 콘텐츠 없음' 기재)와 **틱톡 광고 성인 업종 오분류 재심사**가 정리된 뒤에 켤 것.
