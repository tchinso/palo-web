...
- **⚠️ 남은 문제: 고배율 폰은 lg(720)로도 모자란다.** 정사각이 아닌 그림에서 0.81~0.99배로 살짝 확대된다. 제대로 덮으려면 **lg를 900px로 올리고 기존 `.thumb2`를 재생성**해야 한다(백필에 덮어쓰기 모드 추가 + 판 번호 올리기). 물리적 크기가 작아 체감은 약하지만 규격상 미달인 것은 사실이다.
