...
### 🐛 본인확인 `[CT01] 사이트 설정 오류` — 미해결 (2026-08-13)
인증창까지는 뜨는데 KCP가 **`[CT01] 사이트 설정 오류`**로 되돌린다. **인증이 한 번도 성공한 적이 없다** → `/api/auth/adult-verify`는 아직 실행된 적이 없다(서버 검증 단계는 전부 미검증).
- **⚠️ 우리 코드·포트원 설정은 전부 검증했다. 여기를 다시 뒤지지 말 것.** 배포된 `commi.kr/agegate.js`를 직접 받아 콘솔 값과 1:1 대조한 결과:
...
- **남은 후보**: ①KCP **서비스 개시일 미도래**(승인이 2026-08-12로 하루 전) ②포트원에 저장된 인증키가 KCP 현재 키와 다름(붙여넣기 잘림/구 키) ③KCP 쪽 V2 전환 미반영. **셋 다 KCP 고객센터(1666-6410)로만 확인된다.**
- **문의 시 포트원 쪽을 다 확인했다고 먼저 말할 것** — 안 그러면 "포트원에 문의하세요"로 돌려보낸다.
