...
숨긴 `<input type="color">`로 **OS 스펙트럼 팔레트**를 연다(`pickColor(kind,cb)` 공용) — 폰·PC 모두 원하는 지점을 찍으면 그 색. 직접 그린 팔레트보다 접근성·정확도가 낫다.
- **⚠️ 적용은 change(확정) 한 번만** — input(드래그 중 연발)마다 적용하면 선택 영역에 span이 겹겹이 중첩된다.
