...
- **후속(같은 날)**: 모바일에서 슬라이더를 **4:3**으로 낮춤 — 1:1(375px)이면 통계줄(신청 가능·작업 기간)이 첫 화면 밖으로 31px 밀렸다(실측: 바닥 697 vs 하단 바 위 666). 4:3(281px)으로 94px 확보 → 통계줄까지 스크롤 없이 보이고 63px 여유. contain이라 잘림 없음, PC는 1:1 유지.
